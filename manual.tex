...
\begin{itemize}
    ...
    \item \texttt{final}: disable all changes and commands (commands still exists, they are aliased to no-op functions). The \texttt{final} is also passed to the \texttt{changes} package.
...
\begin{itemize}
    ...
    \item registers \texttt{enrico} as author in the \texttt{changes} package as author, associating the \texttt{zgreen} color to the author's changes (mixed 60\% with red to highlight the change in the text).
\end{itemize}
...
Using the \texttt{changes} package: \added[id=enrico]{This is added text.}
\deleted[id=enrico]{This is deleted text.}
\replaced[id=enrico]{This is a replaced text.}{This is the original text.}
...
\paragraph{Note} This is an advisory lock. Anyone can still modify the section.
